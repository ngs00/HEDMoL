...
class SchNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lin2 = Linear(hidden_channels // 2, self.dim_node_emb)
        ...
        self.attn = Linear(self.dim_node_emb + self.dim_state, 1)
        self.fc_g = Linear(self.dim_node_emb, self.dim_out)
        self.fc_cg = Linear(self.dim_node_emb, self.dim_out)
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.xavier_uniform_(self.fc_g.weight)
        self.fc_g.bias.data.fill_(0)
        torch.nn.init.xavier_uniform_(self.fc_cg.weight)
        self.fc_cg.bias.data.fill_(0)
    ...
    def forward(self, g, u):
        ...
        hg = self.readout(h, batch, dim=0)
        if self.dipole:
            hg = torch.norm(hg, dim=-1, keepdim=True)
        ...
        hs = torch.hstack([h, torch.repeat_interleave(u, g.n_atoms.squeeze(1), dim=0)])
        attns = softmax(self.attn(hs) + 1e-16, g.batch)
        hcg = global_add_pool(attns * h, g.batch)
        hcg = self.fc_cg(hcg)
